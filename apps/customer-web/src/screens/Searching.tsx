...
export function Searching() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => update(id, { status: 'assigned', driver: randomDriver() }), 2600);
    return () => clearTimeout(t);
  }, [id, booking, update]);

  if (!booking) return <Frame title="Booking" back><p className="p-6 text-sm text-neutral-500">Booking not found.</p></Frame>;

  const assigned = booking.status === 'assigned' && booking.driver;
        ...
        {!assigned ? (
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2 rounded-lg bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-700">
              <CheckCircle2 size={16} /> {booking.driver!.name} accepted · arriving in {booking.driver!.etaMin} min
            </div>
            ...
            <div className="rounded-xl border border-neutral-200 bg-white p-4">
              <div className="flex items-center gap-4">
                ...
                <a href={`tel:${booking.driver!.phone}`} className="flex h-11 w-11 items-center justify-center rounded-full bg-emerald-500 text-white">
                  <Phone size={16} />
                </a>
              </div>
              <div className="mt-3 flex items-center justify-between border-t border-neutral-100 pt-3 text-sm">
                ...
                <span className="font-semibold text-neutral-900">{rupees(booking.farePaise ?? 0)}</span>
              </div>
            </div>
