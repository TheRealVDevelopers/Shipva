...
export function PartnerLayout({ title, subtitle, children }: { title: string; subtitle?: string; children: ReactNode }) {
  return (
    <div className="flex h-screen bg-neutral-50">
      {/* dark sidebar — distinct from the light admin console */}
      <aside className="hidden md:flex md:w-64 md:flex-col bg-primary-900 text-white">
        ...
        <div className="m-3 rounded-xl bg-white/[0.06] p-3 ring-1 ring-white/10">
          ...
          <NavLink to="/p/subscription" className="mt-2 block text-[11px] font-bold text-accent-300">Manage plan →</NavLink>
        </div>
      </aside>
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        <header className="flex h-16 items-center justify-between gap-4 border-b border-neutral-200 bg-white px-6">
          ...
          <div className="flex items-center gap-3">
            <span className="hidden sm:inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-3 py-1 text-xs font-bold text-emerald-700 ring-1 ring-inset ring-emerald-200">
