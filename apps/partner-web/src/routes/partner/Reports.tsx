...
import { osCounters, months6, revenueSeries, expenseSeries, expenseBreakdown } from '../../lib/mocks.js';
    ...
    <PartnerLayout title="Reports" subtitle="Profit & loss — by month, vehicle and route">
      <div className="space-y-6">
        <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <KpiCard label="Revenue · MTD" value={inr(osCounters.revenueMtdPaise)} hint="freight billed" tone="primary" icon={<IndianRupee size={14} />} />
          <KpiCard label="Expense · MTD" value={inr(osCounters.expenseMtdPaise)} hint="all costs" tone="accent" icon={<Wallet size={14} />} />
          <KpiCard label="Net profit" value={inr(osCounters.profitMtdPaise)} hint="this month" tone="success" icon={<TrendingUp size={14} />} />
          <KpiCard label="Margin" value={`${osCounters.avgMarginPct}%`} hint="avg per trip" tone="primary" icon={<Percent size={14} />} />
        </section>

        <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <Card className="lg:col-span-2">
            <CardHeader title="Profit & loss" subtitle="Revenue vs expense · 6 months" />
            <CardBody>
              <BarPairChart labels={months6} a={revenueSeries} b={expenseSeries} aLabel="Revenue" bLabel="Expense" aColor="var(--sx-primary-500)" bColor="var(--sx-accent-500)" format={inr} />
            </CardBody>
          </Card>
          <Card>
            <CardHeader title="Cost structure" subtitle="Where money goes" />
            <CardBody><Donut segments={expenseBreakdown} centerMain={inr(osCounters.expenseMtdPaise)} centerSub="expense" /></CardBody>
          </Card>
        </section>
