...
export function printJoiningLetter(v: JoiningParty, opts?: { startDate?: string; endDate?: string }): void {
  ...
  const inner = `
    ...
    <p>${partner.company} is pleased to offer you the opportunity to commence a transportation partnership for the dedicated delivery of goods, utilizing your own fleet vehicles. We anticipate a mutually beneficial and long-term working relationship. To ensure a smooth transition and operational compatibility, this partnership will commence with a mandatory one-week (7 calendar days) Trial Operation Period, governed by the terms below.</p>

    <h2>1. Trial Operation Period (7 Days)</h2>
    <p>The Trial Operation Period will begin on <b>${start}</b> and conclude on <b>${end}</b>. During this period, ${partner.company} will allocate logistics loads to the Vendor and assess overall service performance, reliability, and adherence to scheduling requirements.</p>

    <h2>2. Vendor's Sole Responsibility and Liability (Vehicle Operation)</h2>
    <p>During the trial, the operation and maintenance of the Vendor's vehicle(s) and any associated liabilities shall be the Vendor's sole and exclusive responsibility. ${partner.company} will not be responsible for:</p>
    <ul>
    ...
    <p>During the Trial Period, ${partner.company} will only pay the pre-discussed and agreed transportation service cost for loads successfully delivered. No additional payment will be made for the Vendor's vehicle maintenance, repairs, fuel, insurance or other operational overheads.</p>
    ...
    <div class="sign">
      <div>Sincerely,<br>For <b>${partner.company}</b><div class="line">Name: ____________________<br>Title: ____________________</div></div>
      <div>Acknowledged &amp; Accepted by Vendor<div class="line">Name: ${v.contact || '____________________'}<br>Company: ${v.name}</div></div>
    </div>
    <div class="foot">Generated by ${partner.company} · trial Letter of Intent template.</div>`;

  const w = window.open('', '_blank', 'width=860,height=1000');
  if (!w) { alert('Please allow pop-ups to download the letter.'); return; }
  w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>Joining Letter — ${v.name}</title><style>${CSS}</style></head><body onload="setTimeout(()=>window.print(),200)">${inner}</body></html>`);
  w.document.close();
}
