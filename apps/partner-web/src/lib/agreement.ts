...
export function printAgreement(
  _kind: 'customer' | 'truck-owner',
  vendor: AgreementParty,
  a: Agreement,
  employeeName?: string,
): void {
  ...
  const inner = `
    ...
    <div class="sign">
      <div>For <b>${SARVA.name}</b><div class="line">Name: ${employeeName || '________________________'}<br>Authorised Signatory</div></div>
      <div>For <b>${vendor.name}</b><div class="line">Name: ________________________<br>Authorised Signatory</div></div>
    </div>
    <div class="foot">Generated by ${BRAND.company} · based on the SARVA EXPRESS Service Agreement template. Have your legal team review before signing.</div>`;
